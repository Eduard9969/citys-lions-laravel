                    ...
                    <div class="card-body px-2">
                        @if(!empty($ratings))
                            <table class="table">
                                <thead>
                                <tr>
                                    <th class="border-top-0">{{ __('Place') }}</th>
                                    <th class="border-top-0 text-center">{{ __('Rating') }}</th>
                                    <th class="border-top-0 text-center">{{ __('Date') }}</th>
                                </tr>
                                </thead>
                                @foreach($ratings as $rating)
                                    <tr>
                                        <td><a class="text-body" href="{{ route('places.show', ['place' => $rating['place_id']]) }}">{{ $rating['name'] }}</a></td>
                                        <td class="text-center text-{{ $rating['rating'] > 0 ? 'success' : 'danger' }}">{{ $rating['rating'] > 0 ? __('Up') : __('Down') }}</td>
                                        <td class="text-center">{{ date('Y-m-d H:i', strtotime($rating['created_at'])) }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        @else
